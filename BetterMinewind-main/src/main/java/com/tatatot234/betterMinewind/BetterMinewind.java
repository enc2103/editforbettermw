...
// Bd: Bad dragon Hs: Hollow Sword and so on.
    ...
    private static KeyBinding offhandHotswapBindingBt;
    private static KeyBinding offhandHotswapBindingBd;
    private static KeyBinding offhandHotswapBindingHs;
    private static KeyBinding offhandHotswapBindingSb;
    private static KeyBinding offhandHotswapBindingSl;
    private static KeyBinding offhandHotswapBindingBs;
    private static KeyBinding offhandHotswapBindingOaC;
    ...
    private static KeyBinding antimageHotswapBinding;
    private static KeyBinding SGAHotswapBinding;
    private static KeyBinding aegisHotswapBinding;
    private static KeyBinding muklukHotswapBinding;
    private static KeyBinding BRNHotswapBinding;
    ...
    */
    ...
    private void MwHotswapInit(){
        ...
        offhandHotswapBindingZord   = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Zord Hotswap"         , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingFmb    = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. FMB Hotswap"          , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingBs     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Backstabber Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingBt     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Bloodthorn Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingSl     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Sunlight Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingSb     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Shadowbringer Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingBd     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Bad Dragon Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingHs     = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Hollow Sword Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingOaC    = KeyBindingHelper.registerKeyBinding(new KeyBinding("OF. Orb of Chaos Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingSrage  = KeyBindingHelper.registerKeyBinding(new KeyBinding("HB. Santas Rage Hotswap"  , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        offhandHotswapBindingGc     = KeyBindingHelper.registerKeyBinding(new KeyBinding("HB. Glass Cannon Hotswap" , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));

        elytraHotswapBinding     = KeyBindingHelper.registerKeyBinding(new KeyBinding("CH. Elytra Hotswap"       , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        SGAHotswapBinding        = KeyBindingHelper.registerKeyBinding(new KeyBinding("CH. SGA Hotswap"       , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        antimageHotswapBinding   = KeyBindingHelper.registerKeyBinding(new KeyBinding("CH. Antimage Hotswap"       , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        aegisHotswapBinding      = KeyBindingHelper.registerKeyBinding(new KeyBinding("CH. Aegis Hotswap"        , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        muklukHotswapBinding     = KeyBindingHelper.registerKeyBinding(new KeyBinding("BT. Mukluks Hotswap"        , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
        BRNHotswapBinding     = KeyBindingHelper.registerKeyBinding(new KeyBinding("CH. Byrnhild Hotswap"        , InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, "BetterMinewind"));
    }
    ...
    private void MwHotswapOnTick(MinecraftClient client) {
        ...
        while (antimageHotswapBinding.wasPressed()) {
            int startSlot = 9;
            String antimageLore = "The stout heart knows no fear!";
            ItemStack armor = client.player.getInventory().armor.get(2);
            if (!armor.isEmpty()) {
                if (hasLore(armor)) {
                    //  if we find an antimage already in the chestplate slot, look for another chestplate in the main inventory to swap to.
                    if (armor.getNbt().getCompound(DISPLAY_KEY).get(LORE_KEY).asString().contains(antimageLore)) {
                        int slot = findSlotWithItem(startSlot, 36, "_chestplate", client.player.getInventory());
                        if (slot != -1) {
                            client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                    slot,
                    ...
                    int antimageSlot = findSlotWithLore(startSlot, 36, antimageLore, client.player.getInventory().main);
                    if (antimageSlot != -1) {
                        client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                antimageSlot,
                                38,
        ...
        while (BRNHotswapBinding.wasPressed()) {
            int startSlot = 9;
            String brnLore = "An Armor which is made of";
            ItemStack armor = client.player.getInventory().armor.get(2);
            if (!armor.isEmpty()) {
                if (hasLore(armor)) {
                    //  if we find a BRYN already in the chestplate slot, look for another chestplate in the main inventory to swap to.
                    if (armor.getNbt().getCompound(DISPLAY_KEY).get(LORE_KEY).asString().contains(brnLore)) {
                        int slot = findSlotWithItem(startSlot, 36, "_chestplate", client.player.getInventory());
                        if (slot != -1) {
                            client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                    slot,
                    ...
                    int brnSlot = findSlotWithLore(startSlot, 36, brnLore, client.player.getInventory().main);
                    if (brnSlot != -1) {
                        client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                brnSlot,
                                38,
        ...
        while (SGAHotswapBinding.wasPressed()) {
            int startSlot = 9;
            String SGALore = "Train earnestly, make no compromises.";
            ItemStack armor = client.player.getInventory().armor.get(2);
            if (!armor.isEmpty()) {
                if (hasLore(armor)) {
                    //  if we find an SGA already in the chestplate slot, look for another chestplate in the main inventory to swap to.
                    if (armor.getNbt().getCompound(DISPLAY_KEY).get(LORE_KEY).asString().contains(SGALore)) {
                        int slot = findSlotWithItem(startSlot, 36, "_chestplate", client.player.getInventory());
                        if (slot != -1) {
                            client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                    slot,
                    ...
                    int SGASlot = findSlotWithLore(startSlot, 36, SGALore, client.player.getInventory().main);
                    if (SGASlot != -1) {
                        client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId,
                                SGASlot,
                                38,
        ...
        while(offhandHotswapBindingZord.wasPressed()){
            ...
            if(zordBookSlot != -1){
                ...
                if(zordSwordSlot!= -1){
                    ...
                    client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, zordSwordSlot, 5,
                            SlotActionType.SWAP, client.player);
                }
            }
        }
        ...
        while (offhandHotswapBindingBt.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"Blood for the Blood God!", inventory.main);
            if(fmbSlot != -1){
                //BT to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
        ...
        while (offhandHotswapBindingSl.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"Praise The Sun!", inventory.main);
            if(fmbSlot != -1){
                //Sl to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
        ...
        while (offhandHotswapBindingHs.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"Inside this sword is a world", inventory.main);
            if(fmbSlot != -1){
                //Hs to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
        ...
        while (offhandHotswapBindingSb.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"After the Lightbringer fell into a blackhole,", inventory.main);
            if(fmbSlot != -1){
                //Sb to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
        ...
        while (offhandHotswapBindingBd.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"Overflowing with dragon essence.", inventory.main);
            if(fmbSlot != -1){
                //Bd to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
        ...
        while (offhandHotswapBindingOaC.wasPressed()) {
            PlayerInventory inventory = client.player.getInventory();


            int fmbSlot   = findSlotWithLore(0,36,"Ball of concentrated chaos. Holding", inventory.main);
            if(fmbSlot != -1){
                //OaC to offhand
                client.interactionManager.clickSlot(client.player.playerScreenHandler.syncId, fmbSlot, 40,
                        SlotActionType.SWAP, client.player);
